...
object KafkaSparkCassandra {
  ...
  def main(args: Array[String]) {
    ...
    val sparkConf = new SparkConf().set("spark.cassandra.connection.host", "deepc04.acis.ufl.edu").setAppName("KafkaSparkCassandra")

    // get the values we need out of the config file
    val kafka_broker = "deepc04.acis.ufl.edu:9092"
    val kafka_topic = "test"
    val cassandra_host = "deepc04.acis.ufl.edu" //sparkConf.get("spark.cassandra.connection.host"); //cassandra host
    ...
    session.execute("CREATE KEYSPACE IF NOT EXISTS people WITH REPLICATION = { 'class' : 'SimpleStrategy', 'replication_factor' : 1 };")
    session.execute("CREATE TABLE IF NOT EXISTS people.word_count (word text, ts timestamp, count int, PRIMARY KEY(word, ts)) ")
    session.execute("TRUNCATE people.word_count")
    session.close()
		*/
    ...
    wordCounts.saveToCassandra("people", "word_count", SomeColumns("word" as "_1", "ts" as "_2", "count" as "_3"))
    ...
    val rdd1 = csc.sql("SELECT * from people.word_count") // select the data from the table
